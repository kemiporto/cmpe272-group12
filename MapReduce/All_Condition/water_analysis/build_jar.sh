...
rm *.jar
rm ./build/com/mongodb/hadoop/water/*.class
cp /home/dpham/workspace/Water/bin/com/mongodb/hadoop/water/*.class /home/dpham/Water/water_analysis/build/com/mongodb/hadoop/
cd build
jar cvf $JARNAME *
mv $JARNAME ../
...
declare -a job_args
job_args=("jar" "$HERE/$JARNAME")
job_args=(${job_args[@]} "com.mongodb.hadoop.water.Water")
...
job_args=(${job_args[@]} "-D" "mongo.job.mapper=com.mongodb.hadoop.water.WaterMapper")
job_args=(${job_args[@]} "-D" "mongo.job.reducer=com.mongodb.hadoop.water.WaterReducer")
...
echo "${job_args[@]}" "$1" 
echo "Finished with args"
echo ""

hadoop "${job_args[@]}" "$1" 2>&1 | tee hadoop.out
